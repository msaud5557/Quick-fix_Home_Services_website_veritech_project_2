...
from .forms import CustomerForm,Orderfrom,Servicesform,contact_usform,Orderfromrating
...
class ServiceView(View):
 def get(self ,request):
  as_perr = Services.objects.filter(Employee='Electrician')

#   bottomwers = Services.objects.filter(as_per='BW')
#   mobiles =Services.objects.filter(category='M')
  fott =Services.objects.filter(Employee='Plumber')
...
class CustomerRegistrationView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            messages.info(request, 'Congratulations !! Registered Successfully')
            user=form.save()
            print(form)
...
def viewserviescustomerdash(request, data=None):
    ...
    elif data == 'Electrician' or data == 'Plumber' or data == 'Sweaper'or data == 'Carpenter' or data == 'Drain clean' or data == 'AC repair' or data== 'Painting & Decorating':
        services = Services.objects.filter(Employee=data)
...
def pymentdone(request):
    ...
    if customer:
      ...
      for c in cart:
       ...
       # message1 = ('New order','new order a gaya',  'usamamahar014@gmail.com', ['Saudadmin@gmail.com'])
...
def CustomerFormview(request):
    ...
    if request.method == 'POST':
        ...
        if fm.is_valid():
            ...
            postelcode=fm.cleaned_data['postelcode']
            city=fm.cleaned_data['city']
            Address=fm.cleaned_data['Address']
            image=fm.cleaned_data['customer_image1']
            Customer(user=user,name=name,email=email,whatsapp_no=whattpp,phone_no=phon,mobile=mobile,postelcode=postelcode,state=state,city=city,Address=Address,customer_image1=image).save() 
            ...
            email=request.user.email
            message1 = ('New Customer is Registered', description, 'quickfixpunjabpakistan@gmail.com', ['Saudadmin@gmail.com'])
            message2 = ('wellcome', 'Wellcome to being a part of Quick-FIx', 'quickfixpunjabpakistan@gmail.com', [email])
            send_mass_mail((message1, message2), fail_silently=False)
...
def addservices(request):
    ...
    if request.method == 'POST':
        ...
        if fm.is_valid():
            ...
            address=fm.cleaned_data['address']
            city=fm.cleaned_data['city']
            Employee=fm.cleaned_data['Employee']
            Province=fm.cleaned_data['Province']
            Cnic=fm.cleaned_data['Cnic']
            Shop_References=fm.cleaned_data['Shop_References']
            Service_image1=fm.cleaned_data['Service_image1']
            # Service_image2=fm.cleaned_data['Service_image2']
            Services(user=user,name=name,phone_no=phone_no,Employee=Employee,mobile_no=mobile_no,address=address,city=city,Province=Province,Cnic=Cnic,Shop_References=Shop_References,Service_image1=Service_image1).save()
       ...
       Subject,
    description,
    'quickfixpunjabpakistan@gmail.com',
    ['Saudadmin@gmail.com'],
    fail_silently=False,
)
...
def countcustomer(request):
    ...
    order=OrderPlaced.objects.filter(status='Service is Start').count()
    ...
    order1=OrderPlaced.objects.filter(status='Cancel').count()
    ...
    return render(request, 'app/count.html',{'order1':order1,'orderscom':orderscom,'orders':orders,'msg':msg,'msg_count':msg_count,'customer':customer,'order':order,'total_services':total_services})
    ...
    if request.method == 'POST':
       ...
       # text_content = 'This is an important message.'
        ...
        email=EmailMultiAlternatives(
            Subject,text_content,'quickfixpunjabpakistan@gmail.com',[email]
        ...
        send_mail(
      Subject,
    "A massage is come check you website account",
    'quickfixpunjabpakistan@gmail.com',
...
# order update by cutomer
def updateorderbycustomer(request,id):

    if request.method=='POST':
        pi=OrderPlaced.objects.get(pk=id)
        
        
        fm=Orderfromrating(request.POST,instance=pi)
        if fm.is_valid():
            fm.save()
          ...
          fm=Orderfromrating(instance=pi)  
    return render(request,'app/orderupdatebycutomer.html',{'form':fm })
